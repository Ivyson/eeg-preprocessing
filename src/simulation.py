...
class EEGVisualization(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.psd_freq_limit = 60.0  # Hz to show on PSD plot
    ...
    def setup_ui(self):
        ...
        for i in range(self.n_channels):
            pen = pg.mkPen(
                color=self.colors[i], width=1, style=QtCore.Qt.PenStyle.DashLine
            )
            c = self.psd_widget.plot(
                [], [], pen=pen, name=self.channel_names[i], alpha=0.5
            )
            self.psd_curves.append(c)
        # Avg PSD (bold)
        self.psd_avg_curve = self.psd_widget.plot([], [], pen=pg.mkPen("k", width=2))

        # Legend toggle button to collapse/expand legend panel
    ...
    def setup_headplot(self):
        """Initial headplot setup if any (kept empty for now)."""
        pass
    ...
    def compute_and_update_psd(self):
        """Compute PSD for each channel from rolling buffer and update PSD plots."""
        # Use current signal_buffer: shape (n_channels, buffer_size)
        psd_list = []
        freq = None
        for i in range(self.n_channels):
            # If channel is not visible we can still compute PSD but set visibility off
            f, Pxx = welch(
                self.signal_buffer[i],
                fs=self.sample_rate,
                nperseg=min(self.psd_nperseg, self.buffer_size),
            )
            if freq is None:
                freq = f
            # convert to dB
            ...
            # update individual PSD curve if visible
            # limit freq to psd_freq_limit
            idx = freq <= self.psd_freq_limit
            self.psd_curves[i].setData(freq[idx], Pxx_db[idx])

        # average PSD (mean across channels)
        avg = np.mean(psd_list, axis=0)
        idx = freq <= self.psd_freq_limit
        self.psd_avg_curve.setData(freq[idx], avg[idx])
    ...
    def update_plots(self):
        ...
        # update each overlayed (stacked) curve
